...
> **2026-09-23 변경:** 위 내용은 초기 단일 파일 버전에 대한 분석 기록이다. 신규 맵 5종 정식 통합 이후에는 index.html과 maps/*.js를 함께 HTTP 정적 호스팅해야 하며, file:// 직접 실행과 index.html 단독 배포는 지원하지 않는다.
...
## 17. 메챠 카멜레온 오마주 3단계 로드맵 수립 및 1단계(지우개 삑삑이 & 도발 시스템) 구현 완료 (v2.2.0)

### 17.1 원조 게임(메챠 카멜레온 / MECCHA CHAMELEON) 연구 성과
- 2026년 6월 스팀 글로벌 메가히트작 '메챠 카멜레온'의 핵심 게임성(스포이드 페인팅, 포즈, 파묻힘 방지, 자동 휘파람, 감염 모드, 미술관 맵) 분석 완료.
- 사용자(교사)의 피드백에 따라 **1단계(자동 소리 알림) → 2단계(술래 증식 감염 모드) → 3단계(신규 미술실 맵)**로 이어지는 체계적 로드맵 수립.

### 17.2 1단계: 지우개 삑삑이 (Auto-Whistle) & 수동 도발 시스템 구현
1. **절차적 삑삑이 사운드 (`Audio2.sfxSqueak`) 탑재**:
   - Web Audio API를 활용한 고무 지우개 특유의 고주파 귀여운 삑삑이 소리(950Hz → 1550Hz) 합성.
   - 3D 공간 거리(`distanceTo`)에 따른 지능형 볼륨 감쇄 적용으로 술래에게 직관적인 청각 방향/거리 힌트 제공.
2. **자동 삑삑이 (Auto-Whistle) 주기 발동**:
   - `SEEK` 페이즈 진행 중 30초 주기(90초, 60초, 30초, 10초 남은 시점)마다 모든 생존 지우개의 위치에서 삑삑이 사운드와 별가루 파티클(`burst`) 동시 분출.
   - 지우개 머리 위 3D 월드 좌표에 "🎵 삑!" 팝업 연출 및 본인 화면에 "🤫 삑! 내 지우개에서 소리가 났어요!" 토스트 안내.
   - 술래 화면에 "👂 삑! 어디선가 지우개 소리가 들렸어요!" 알림 제공.
3. **수동 도발 버튼 (`btnTaunt` & `KeyT`) 탑재**:
   - 숨어있는 지우개 학생이 가만히 멍때리지 않고 심리전을 걸 수 있도록 모바일 `[🎵 삑!]` 버튼 및 PC `T` 단축키 제공.
   - 쿨다운 8초. 술래가 14m 이내에 있을 때 대담하게 도발 성공 시 "🔥 대담한 도발 성공! 술래를 놀렸어요! (+50점)" 팡파르 및 가산점 부여.
4. **MVP 칭찬 뱃지 '대담한 삑삑이 상' 연동**:
   - 라운드 종료 결과 창에서 술래 근처 도발에 성공한 학생을 자동으로 표창.

---

## 18. v2.2.1 모바일 이동 조작 회귀 버그 수정

### 18.1 현장 증상
- 게임 중 화면 왼쪽에서 이동 조이스틱이 생성되지 않거나 움직임이 반영되지 않는 간헐적 현상이 재발함.

### 18.2 원인
1. v2.2.0의 수동 도발 버튼이 `left:18px; bottom:80px`에 추가되어 왼쪽 45% 이동 터치 영역을 직접 차단함.
2. 왼쪽 하단의 포즈 버튼도 `pointer-events:auto`로 이동 터치 시작을 캔버스에 전달하지 않음.
3. 조이스틱을 누른 채 색칠 모드로 전환하면 종료 이벤트가 누락된 경우 `joyId`가 남아 다음 이동 터치 등록을 막을 수 있음.
4. 교사의 '얼음!'은 의도된 이동 제한이지만, 2초 안내 후에는 조이스틱 고장으로 오인할 수 있음.

### 18.3 적용 내용
1. 왼쪽 45% 영역을 부동 조이스틱 전용 공간으로 예약하고 도발·포즈 버튼을 `left:calc(45% + 8px)` 기준의 오른쪽 조작 그룹으로 이동.
2. `setMirror()` 시작과 '얼음!' 전환 시 `resetMovementInputs()`를 호출해 포인터 ID, 이동값, 비행 입력을 정리.
3. '얼음!' 상태에서 이동 영역을 누르면 제어 상태를 알리는 토스트를 표시.

### 18.4 검증 체크리스트
- [ ] iPad Safari 가로 화면에서 왼쪽 하단·중단 조이스틱 생성 및 이동
- [ ] 왼손 이동 + 오른손 시점 회전/점프/스포이드 동시 조작
- [ ] 조이스틱을 누른 채 색칠 모드 진입·종료 후 이동 재개
- [ ] '얼음!' 실행·해제 후 이동 재개와 안내 토스트
- [ ] 술래·지우개·유령·관전 역할별 모바일 조작

### 18.5 2026-09-21 PC 사전 검증 결과
- [x] Chrome 터치 입력 환경의 오프라인 연습 모드 진입 및 모바일 조작 UI 표시
- [x] 왼쪽 45% 이동 영역과 도발·포즈 버튼 영역의 코드·화면 배치 분리 확인
- [x] 색칠 모드 진입 시 이동 UI가 숨겨지고, 종료 후 이동·점프·스포이드·포즈 UI가 복귀하는 흐름 확인
- [x] `setMirror()`와 '얼음!' 전환 경로에서 `resetMovementInputs()` 호출 확인
- [x] '얼음!' 상태 이동 터치 안내 토스트 및 1.5초 중복 방지 로직 확인
- [x] `index.html`의 모듈 JavaScript 문법 검사 통과(Node.js v24.15.0)
- [x] Chrome 실행 중 브라우저 콘솔 경고·오류 없음
- [ ] 실제 iPad Safari의 멀티터치 및 교사·학생 Firebase 다중 접속 검증

> 참고: GitHub Pages 루트 배포 파일인 `index.html`이 최신 구현입니다. 별도 보관본 `index (배포용).html`과 `index (배포용).zip`은 v2.2.1 모바일 핫픽스와 v2.3.0 증식 모드가 반영되지 않은 이전 산출물이므로 최신 배포본으로 사용하지 않습니다.

---

## 19. v2.3.0 교실 맞춤형 후반 술래 증식 모드

### 19.1 적용 근거
- 20명 등교 시 최초 술래 4명, 21명 참여 시 최초 술래 5명으로 운영했으나 5회 동안 전원 검거가 한 번도 발생하지 않음.
- 최초 술래 수 부족보다는 마지막 생존자 탐색과 전원 검거 승리 조건이 술래에게 지나치게 어려운 것으로 판단.

### 19.2 확정 규칙
- 로비에서 선생님이 `베이직` 또는 `감염` 모드를 카드로 선택하며 새 방의 기본값은 베이직.
- 더블 모드는 후속 구현 대상으로 선택 불가능한 준비 중 카드로 표시.
- 숨는 시간 60초와 찾는 시간 180초를 유지.
- 베이직은 약 4명당 1명(20명 5명, 21명 6명), 감염은 약 5명당 1명(20명 4명, 21명 5명)을 최초 술래로 자동 배정.
- 감염 시작은 원작식 `즉시`가 기본이며 선생님이 30초·60초 후 시작도 선택할 수 있음.
- 감염된 지우개는 5초 대기 후 빨간 연필 술래로 전환되어 남은 지우개를 검거할 수 있음.
- 지연 감염을 선택했을 때 활성화 전에 잡힌 지우개는 기존처럼 유령 역할을 유지.
- 승패·생존자·통계는 라운드 시작 당시 지우개 명단을 기준으로 판정하여 역할 전환 후에도 누락되지 않음.

### 19.3 현장 검증 항목
- [ ] 20명 경기에서 최초 술래 4명·지우개 16명 배정 확인
- [ ] 21명 경기에서 최초 술래 5명·지우개 16명 배정 확인
- [ ] 베이직 20명 경기에서 최초 술래 5명 배정 확인
- [ ] 베이직 21명 경기에서 최초 술래 6명 배정 확인
- [ ] 감염 시작 `즉시·30초·60초` 선택값이 각 라운드에 반영되는지 확인
- [ ] 감염 활성화 이후 검거자는 5초 카운트다운 뒤 빨간 연필로 전환되는지 확인
- [ ] 증식 술래가 남은 지우개를 정상적으로 검거하는지 확인
- [ ] 감염자가 포함된 상태에서도 전원 검거 즉시 술래 승리로 종료되는지 확인
- [ ] 결과 화면의 생존자 수와 검거 통계가 정확한지 확인

### 19.4 자동 수집 지표
- 라운드 모드와 감염 시작 시각
- 최초 술래 수와 감염 술래 수
- 첫 발견까지 걸린 시간
- 찾기 시작 60초·120초 시점의 생존 지우개 수
- 최종 발견 수와 전원 검거 시각
- 세부 시험 절차: [`02_game_mode_field_test.md`](./02_game_mode_field_test.md)
