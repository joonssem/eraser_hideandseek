...
1. **미술실 (`art_room.js`)**: 팔레트, 크레파스함, 12단 작품 건조대, 조소용 찰흙
2. **체육관 (`gymnasium.js`)**: 농구 코트 마루, 단상 무대, 뜀틀, 늑목, 볼 카트
3. **음악실 (`music_room.js`)**: 그랜드 피아노, 보면대 합창석, 팀파니, 실로폰
4. **보건실 (`health_office.js`)**: 침대 3베드 커튼, 신장체중계, 약품장, 시력검사표
